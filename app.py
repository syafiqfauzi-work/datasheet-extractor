...
if uploaded_file is not None:
    if st.button("Extract Data", type="primary"):
        ...
        try:
            ...
            - FOR "Pitch_Calculation_Logic": 1) Locate the main "Dimensions" table for the target MPN. 2) Extract the nominal Length (L) in mm. 3) CRITICAL: Check if the Terminal Width/Electrode dimension (labeled T, A, b, etc.) changes based on the "Resistance Range". If it does, evaluate your specific component's resistance (e.g., 0.3 Ohm) against those ranges to select the correct nominal mm terminal width. 4) Calculate Pitch using: Pitch = Length - Terminal Width. Write out the evaluation step-by-step (e.g., "0.3 falls in 0.007 to 0.5, so T=0.508. Pitch = 5.08 - 0.508 = 4.572").
