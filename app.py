    ...
    if st.button("Extract Data", type="primary"):
        with st.spinner("Reading PDF and extracting data... Please wait."):
            try:
                ...
                - FOR "Temperature Coefficient": CRITICAL LOGIC. T.C.R. often depends on BOTH Tolerance and Resistance ranges. You MUST perform this 2-step verification: 1) Identify the Tolerance code from the MPN (e.g., 'F' = 1%, 'J' = 5%) and locate that specific section in the T.C.R table. 2) Convert the MPN resistance to a decimal (e.g., 5R11 = 5.11 Ohms) and mathematically evaluate which specific range it falls into within that exact Tolerance block (e.g., 5.11 is ≤ 10, so it matches 1Ω ≤ R ≤ 10Ω, NOT >10Ω). Extract the specific T.C.R. for that exact matched tier. Return value and unit (e.g., "200 ppm/°C"). Discard "±".
