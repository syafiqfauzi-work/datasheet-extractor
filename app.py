    ...
    if st.button("Extract Data", type="primary"):
        with st.spinner("Reading PDF and extracting data... Please wait."):
            try:
                ...
                - FOR "Temperature Coefficient": This may be labeled as "T.C.R.", "TCR", or "T. C. R.". Discard any "±" symbols.
                  * Note 1: If the T.C.R. value depends on Resistance ranges (e.g., 1Ω ≤ R ≤ 10Ω), you MUST follow this strict logic: 1) Identify the exact numeric resistance in Ohms from the MPN (e.g., 5R11 means 5.11 Ω). 2) Mathematically evaluate which range this exact decimal number falls into (e.g., 5.11 is ≤ 10, so it belongs in the 1Ω ≤ R ≤ 10Ω tier, NOT the >10Ω tier). 3) Extract the specific T.C.R. assigned to that matched tier. Extract the numerical value TOGETHER WITH its exact unit (e.g., "200 ppm/°C").
